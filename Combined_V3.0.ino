...
void loop() {
  ...
  if(switchTest){
    ...
    for(int i = 10; i > 0; i--){
      int ldr = analogRead(A0);
      int ldrMapped = map(ldr, 0, 1023, 0, 255);
      analogWrite(ledWhite, ldrMapped);
      analogWrite(ledOrange, ldrMapped);
      lcd.setCursor(6, 1);
      lcd.print(ldr);
    ...
    lcd.setCursor(16, 2); 
    lcd.print(triggerValue3);
    Serial.println("  triggerValue3 SET"); 
    ...
    Serial.println("  triggerValue2 SET"); 
    ...
    Serial.println("  triggerValue1 SET"); 
    Serial.println("===== FINISHED SETUP =====");

    lcd.setCursor(0, 0);
    lcd.print("/------------------\\");
    lcd.setCursor(0, 1);
    lcd.print("|  Change switch    |");
    lcd.setCursor(0, 2);
    lcd.print("|  In    Seconds    |");
    lcd.print("| Exiting Demo mode |");
    lcd.setCursor(0, 3);
    lcd.print("\\------------------/");
    ...
    for(int i = 0; i < 10; i++){
      if(switchDemo == LOW){
        ...
        lcd.setCursor(0, 2);
        lcd.print("| Exiting Demo mode |");
        Serial.println("  Went out of demo mode");
        delay(2000);
        break;
      }
    ...
    mainLoop(ldr, ldrMapped, triggerValue1, triggerValue2, triggerValue3);
  }
...
int mainLoop(int ldr, int ldrMapped, int triggerValue1, int triggerValue2, int triggerValue3) {
  if(ldr < triggerValue1){
    ...
    while(ldr < triggerValue2){
      ...
      if(analogRead(A0) > triggerValue2){ Serial.println("  - broke out of while loop, cause: ldr change"); break; } 
    }
